...
class CityApi 
{
	...
	// Jim Camomile switched this to false
	public  $json = false;                       // Should results be JSON or PHP associative array?
	// Jim Camomile switched this false for testing
	public  $ssl_verify_peer = false;            // Should curl verify it's SSL peer? Leave true in production, you can set to false to in some test environments, like XAMPP on Windows
...
// Added by Jim Camomile
	public function events_index($args = NULL)
	{
		try {	
			$this->debug_message (__FUNCTION__);
			
			$url = CITYAPIBASEURL . '/events';
			if(!is_null($args))
				$url = $this->add_querystring($url, $args);
